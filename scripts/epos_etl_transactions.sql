...
-- what so ever we do not load the tables with transactions that 
-- have been loaded already.


-- As the delete is not based on a Primary Key we induce an artificial predicate
delete a
from srcm_nbks_trans_summary as a, srcm_nbks_epos_trans_staging as b
where a.trans_no = b.trans_no  and a.primary_id <> 0 ;
 
-- As the delete is not based on a Primary Key we induce an artificial predicate 
delete a
from srcm_nbks_trans_detail as a, srcm_nbks_epos_trans_staging as b
where a.trans_no = b.trans_no and a.primary_id <> 0 ;
...
select substr(trans_no,1, instr(trans_no,'-') - 1),
       ...
       CASE 
       	   WHEN substring(trans_memo,1,9) NOT IN ('STOCKTRFR', 'ONLINECHK', 'ONLINECRD', 'MAILORCHK', 'MAILORCRD', 'CRSUBSCRP', 'STOCKADDN')
           	THEN 'STORESALE'
	   ELSE 
