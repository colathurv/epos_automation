...
for /f "eol=# tokens=1,2* delims==" %%i in (%property_file%) do if "%%i" == "INVSTORECODES"  set INVSTORECODES=%%j
echo %SCRIPTHOME%
echo %TEMPHOME%
echo %STORECODES%
...
for /F %%i in (%INVSTORECODES%) do (
  ...
  type %SCRIPTHOME%\epos_update_inventory.sql >> %TEMPHOME%\temp2.txt
  
  REM Executing the Script to create the reports
  mysql --defaults-file=%SCRIPTHOME%\srcm.cnf -s -usrcmadmin -e "source %TEMPHOME%\temp2.txt" -v --show-warnings
