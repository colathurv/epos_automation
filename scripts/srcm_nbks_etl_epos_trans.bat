...
for /F %%S in (%STORECODES%) do (
	...
	for %%v in (%EPOSLZHOME%\%%S\*.csv) do (
		...
		echo Ready to load ...
		mysql --defaults-file=%SCRIPTHOME%\srcm.cnf -v -usrcmadmin -e "source %TEMPHOME%\temp1.txt"
...
mysql --defaults-file=%SCRIPTHOME%\srcm.cnf -s -usrcmadmin -e "source %SCRIPTHOME%\epos_etl_transactions.sql"
